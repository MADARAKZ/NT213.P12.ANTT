...
function getUserIdFromToken(req) {
  ...
  try {
      ...
      const decoded = jwt.verify(token, JWT_SECRET); // Xác thực và giải mã token
      return decoded.userId; // Trả về userId từ payload của token
  } catch (err) {
      throw new Error("Token is invalid or expired");
  }
}

module.exports = { authenticateToken, requireAdmin, requireCustomer, requireOwner, getUserIdFromToken };