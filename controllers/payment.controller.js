...
const deleteBookingWithStatusFalse = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    // Kiểm tra nếu status không phải là false
    if (booking.status !== false) {
      return res
        .status(400)
        .send({ message: "Booking cannot be deleted as its status is not false" });
    }

    // Xóa booking nếu status là false
