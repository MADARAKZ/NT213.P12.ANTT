$(document).ready(async function () {
  ...
  function validateAndSendBookingRequest() {
    ...
    if (!address || !/[a-zA-Z0-9]/.test(address)) {
      isValid = false;
      errorMessage +=
        "Địa chỉ không được để trống và phải chứa ít nhất một ký tự.\n";
    }
    const currentYear = new Date().getFullYear();
    if (
      !/^\d{4}$/.test(yearOfBirth) ||
      yearOfBirth < 1900 ||
      yearOfBirth > currentYear
    ) {
      errorMessage +=
        "Năm sinh không hợp lệ. Vui lòng nhập năm từ 1900 đến hiện tại.\n";
    }
    const nameRegex =
      /^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪỬỮỰỲỴÝỶỸỲỳỵỷỹ\s]+$/;
    if (!nameRegex.test(fullNameTest)) {
      isValid = false;
      errorMessage +=
        "Họ và tên chỉ được chứa chữ cái, dấu tiếng Việt và khoảng trắng.";
    }
...
function validateCCCD(cccd) {
  ...
  // Danh sách mã tỉnh/thành phố đầy đủ
